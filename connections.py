...
class FlaskApp:
    ...
    def initialise_scraper(self, link, overall_review, feature_review, sub_feature_checkboxes):
        ...
        try:
            ...
            if files:
                return {'success': True, 'message': 'Partially scraped... \nChromedriver Error Occurred.'}
            else:
            ...
            if thread1 is not None:
                ...
                '''                if isinstance(feature_sentiment_file_list, list):
                    for feature in feature_sentiment_file_list:
                        visualisations.GeographicalVisualisation(feature, folder_name)'''
    ...
    def serve_html(self):
        global folder_name
        folder_name = ''
        return send_from_directory(self.file_path, 'sentiment_analysis_home.html')

    def serve_analysis_page(self):
        global folder_name
        # print(peak_month['Review_Date'], peak_month['Review_Count'])
        brand = request.args.get('brand')
        model = request.args.get('model')
        ...
        if not folder_name or not(brand and model):
        ...
        print(f'{folder_name = }')
        print(f'{self.text_values = }')
        self.start_visualisations(geodata=False)
        return render_template('analysis_and_visualisation.html', star_counts=self.text_values)
    ...
    def serve_load_csv(self):
        ...
        return render_template('view_csv.html', files=files)

    def geographical_visualisation(self):
        return render_template('geographical_visualisation.html')
